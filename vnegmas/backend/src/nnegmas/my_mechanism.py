...
from negmas.outcomes import (Outcome, ResponseType, outcome_as_dict,
                             outcome_is_valid, sample_outcomes)
from negmas.utilities import (JavaUtilityFunction, MappingUtilityFunction,
                              UtilityFunction, UtilityValue, normalize)
...
class AspirationNegotiator(MyNegotiator, AspirationMixin):
    ...
    def _update_ordered_outcomes(self):
        ...
        if not self.assume_normalized:
            self.utility_function = normalize(
                self.utility_function, outcomes=outcomes, infeasible_cutoff=-1e-6
            )
