...
def negmas_layout(G, layer_sizes):
    ...
    left, right, bottom, top, layer_sizes = 0.1, 0.9, 0.3, 0.9, layer_sizes
    ...
    for i, v in enumerate(layer_sizes):
        layer_top = v_spacing * (v - 1) / 2.0 + (top + bottom) / 2.0
        for j in range(v):
            pos.append(
                np.array(
                    [left + i * h_spacing, layer_top - j * v_spacing], dtype="float64"
                )
            )
            node_count += 1
...
def negmas_add_nodes(G, layer_sizes, node_name=None):
    ...
    left, right, bottom, top, layer_sizes = 0.1, 0.9, 0.27, 0.73, layer_sizes
    v_spacing = (top - bottom) / float(max(layer_sizes))
    h_spacing = (right - left) / float(len(layer_sizes) - 1)
    node_count = 0
    A = nx.to_numpy_array(G)
...
def negmas_add_edges(G, layer_sizes, node_name=None, contracts=None):
    for x, (left_nodes, right_nodes) in enumerate(
...
if __name__ == "__main__":
    ...
    node_name = [
        ["m_1", "m_2", "m_3"],
        ["my@1_2", "my@1_3", "_df@1_4"],
        ["_df_1", "_df_2", "_df_3"],
        ["greedy@2_2", "greedy@1_2", "greedy@2_3"],
        ["c_0", "c_1", "c_2"],
    ]

    layer_sizes = [len(layer) for layer in node_name]
    contracts = [
        (node_name[0][0], node_name[1][0]),
        (node_name[1][0], node_name[2][2]),
        (node_name[2][2], node_name[3][0]),
        (node_name[3][0], node_name[4][2]),
    ]
    import random
