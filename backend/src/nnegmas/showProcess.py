...
class ShowProcess:
    ...
    def __init__(
        ...
        ax=None,
        fig=None,
        world_recall_reuslt_dict=None,
        draw_backend="matplotlib",
        mode="online_memory",
        *args,
        **kwargs
    ):
    ...
    def iter_frame(self):
        ...
        while True:
            step_information = (
                self._world_recall_reuslt_dict["current_step"]
                if "current_step" in self._world_recall_reuslt_dict
                else 0
            )
    ...
    def init(self):
        ...
        if self.draw_backend == "matplotlib":
            ...
            negmas_draw.negmas_draw(
                self.g,
                negmas_draw.negmas_edge_colors,
                node_colors=negmas_draw.negmas_node_colors,
                pos=self.pos,
            )
    ...
    def show(self):
        ai = FuncAnimation(
            self.fig, self.update, frames=self.iterf, interval=100, init_func=self.init
        )
...
class AnimatedScatter(object):
    ...
    def update(self):
        ...
        try:
            ...
            print(node_color)
            for i in range(1):
                dnx.draw_networkx_nodes(
                    self.show.g, node_color=node_color, pos=self.show.pos
                )
                # negmas_draw.negmas_draw(self.show.g, negmas_draw.negmas_edge_colors, node_colors=node_color, pos=self.show.pos)
                time.sleep(0.2)
                dnx.draw_networkx_nodes(
                    self.show.g, node_color="red", pos=self.show.pos
                )
                time.sleep(0.2)
                dnx.draw_networkx_nodes(
                    self.show.g, node_color=back_node_color, pos=self.show.pos
                )
